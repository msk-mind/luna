...
def test_cli(tmp_path, monkeypatch):
    def mock_get(*args, **kwargs):
        ...
        if args[1] == '/collection?text=test_collection':
            return [{'name': 'test-collection', '_id': 'uuid', '_modelType':'collection'}]
    ...
    def mock_listCollection(*args, **kwargs):
        return [{'name': 'test-collection', '_id': 'c-0001', '_modelType':'collection'}]

    def mock_listResource(*args, **kwargs):
        return [{'name': 'test-resource', '_id': 'r-0001', 'largeImage': np.nan, '_modelType':'resource'}] # A way to mock 0 slides
