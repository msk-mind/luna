...
import pandas as pd
import pydicom
from pydicom import Dataset, FileDataset, datadict
...
def get_image_directory_meta_data(image_folder, subject):
    ...
    for image_file in image_file_list:
        ...
        image_file_type = get_image_type(
        ...
        if image_file_type == "dicom":
            ...
            try:
                ...
                logging.warning(
                    "DICOM header could not be read for %s.",
                    os.path.join(image_folder, image_file),
                )
                dcm = None
        else:
        ...
        # Add file_path
        image_meta_list = [
            pd.Series({"file_path": os.path.join(image_folder, image_file)})
        ]
...
def get_meta_data(modality, dcm_list=None, image_folder=None):
    ...
    if dcm_list is None and image_folder is None:
        raise ValueError(
            "One of dcm_list and image_folder parameters needs to be provided."
        )
...
def set_pydicom_meta_tag(
    dcm_seq: Union[FileDataset, Dataset], tag, value, force_vr=None
):
    # Check tag
    if isinstance(tag, tuple):
        ...
        raise TypeError(
            f"Metadata tag {tag} is not a pydicom Tag, or can be parsed to one."
        )
...
def get_series_meta_data(image_file=None, dcm=None, shorten_uid=False):
    ...
    if dcm is not None:
        ...
        study_instance_uid = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0020, 0x000D), tag_type="str", default=""
        )

        # Read series instance UID
        series_instance_uid = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0020, 0x000E), tag_type="str", default=""
        )

        # Frame of reference UID
        if get_pydicom_meta_tag(dcm_seq=dcm, tag=(0x3006, 0x010), test_tag=True):
            frame_of_reference_uid = get_pydicom_meta_tag(
                dcm_seq=dcm[0x3006, 0x010][0],
                tag=(0x0020, 0x0052),
                tag_type="str",
                default="",
            )
            ...
            frame_of_reference_uid = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0020, 0x0052), tag_type="str", default=""
            )

        # Read patient name
        patient_name = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0010, 0x0010), tag_type="str", default=""
        )

        # Read study description
        study_description = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x1030), tag_type="str", default=""
        )

        # Read series description
        series_description = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x103E), tag_type="str", default=""
        )

        # Examined body part
        body_part_examined = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x0015), tag_type="str", default=""
        )

        # Read study date and time
        study_start_date = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x0020), tag_type="str"
        )
        study_start_time = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x0030), tag_type="str", default=""
        )

        # Read acquisition date and time
        acquisition_start_date = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x0022), tag_type="str"
        )
        acquisition_start_time = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x0032), tag_type="str", default=""
        )
...
def get_basic_image_meta_data(image_file=None, dcm=None):
    ...
    if dcm is not None:
        # Modality
        modality = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x0060), tag_type="str"
        )

        # Instance number
        instance_number = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0020, 0x0013), tag_type="int", default=-1
        )

        # Scanner type
        scanner_type = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x1090), tag_type="str", default=""
        )

        # Scanner manufacturer
        manufacturer = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0008, 0x0070), tag_type="str", default=""
        )

        # Slice thickness
        spacing_z = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0018, 0x0050), tag_type="float", default=np.nan
        )

        # Pixel spacing
        pixel_spacing = get_pydicom_meta_tag(
            dcm_seq=dcm, tag=(0x0028, 0x0030), tag_type="mult_float"
        )
...
def get_basic_ct_meta_data(image_file=None, dcm=None):
    ...
    meta_data = pd.Series({"image_type": "", "kvp": np.nan, "kernel": "", "agent": ""})

    if dcm is not None:
        if (
            get_pydicom_meta_tag(dcm_seq=dcm, tag=(0x0008, 0x0060), tag_type="str")
            == "CT"
        ):

            # Image type
            image_type = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0008, 0x0008), tag_type="str", default=""
            )

            # Peak kilo voltage output
            kvp = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0060), tag_type="float", default=np.nan
            )

            # Convolution kernel
            kernel = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x1210), tag_type="str", default=""
            )

            # Contrast/bolus agent
            contrast_agent = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0010), tag_type="str", default=""
            )
...
def get_advanced_ct_meta_data(image_file=None, dcm=None):
    ...
    meta_data = pd.Series(
        {"tube_current": np.nan, "exposure_time": np.nan, "exposure": np.nan}
    )
    ...
    if dcm is not None:
        if (
            ...
            tube_current = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x1151), tag_type="float", default=np.nan
            )

            # Exposure time in milliseconds
            exposure_time = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x1150), tag_type="float", default=np.nan
            )

            # Radiation exposure in mAs
            exposure = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x1152), tag_type="float", default=np.nan
            )
    ...
    if dcm is not None:
        if (
            ...
            image_corrections = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0028, 0x0051), tag_type="str", default=""
            )

            # Randoms correction method
            random_correction_method = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0054, 0x1100), tag_type="str", default=""
            )

            # Attenuation correction method
            attenuation_correction_method = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0054, 0x1101), tag_type="str", default=""
            )

            # Scatter correction method
            scatter_correction_method = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0054, 0x1105), tag_type="str", default=""
            )

            # Reconstruction method
            reconstruction_method = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0054, 0x1103), tag_type="str", default=""
            )
            ...
            if get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0054, 0x0016), tag_type=None, test_tag=True
            ):
    ...
    if dcm is not None:
        if (
            ...
            acquisition_ref_time = convert_dicom_time(
                date_str=get_pydicom_meta_tag(
                    dcm_seq=dcm, tag=(0x0008, 0x0022), tag_type="str"
                ),
                time_str=get_pydicom_meta_tag(
                    dcm_seq=dcm, tag=(0x0008, 0x0032), tag_type="str"
                ),
            )
            ...
            if radio_admin_ref_time is None:
                ...
                acquisition_ref_time = convert_dicom_time(
                    get_pydicom_meta_tag(
                        dcm_seq=dcm, tag=(0x0009, 0x100D), tag_type="str"
                    )
                )
                radio_admin_ref_time = convert_dicom_time(
                    get_pydicom_meta_tag(
                        dcm_seq=dcm, tag=(0x0009, 0x103B), tag_type="str"
                    )
                )
            ...
            if radio_admin_ref_time is not None and acquisition_ref_time is not None:
                ...
                uptake_time = (
                    acquisition_ref_time - radio_admin_ref_time
                ).seconds / 60.0
            else:
                uptake_time = np.nan
            ...
            frame_duration = (
                get_pydicom_meta_tag(
                    dcm_seq=dcm, tag=(0x0018, 0x1242), tag_type="float", default=np.nan
                )
                / 1000.0
            )

            # Important data for SUV normalisation
            patient_gender = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0010, 0x0040), tag_type="str"
            )
            patient_weight = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0010, 0x1030), tag_type="float"
            )
            patient_height = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0010, 0x1020), tag_type="float"
            )

            # The type of intensity represented by the pixels
            intensity_unit = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0054, 0x1001), tag_type="str"
            )
            ...
            time_of_flight = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x9755), tag_type="str", default=""
            )

            # Read reconstruction sequence (0018,9749)
            if get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x9749), tag_type=None, test_tag=True
            ):
                # Reconstruction type (0018,9749)(0018,9756)
                recon_type = get_pydicom_meta_tag(
                    dcm_seq=dcm[0x0018, 0x9749][0],
                    tag=(0x0018, 0x9756),
                    tag_type="str",
                    default="",
                )
    ...
    if dcm is not None:
        if (
            ...
            scanning_sequence = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0020), tag_type="str", default=""
            )

            # Scanning sequence variant
            scanning_sequence_variant = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0021), tag_type="str", default=""
            )

            # Sequence name
            scanning_sequence_name = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0024), tag_type="str", default=""
            )

            # Scan options
            scan_options = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0022), tag_type="str", default=""
            )

            # Acquisition type
            acquisition_type = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0023), tag_type="str", default=""
            )

            # Repetition time
            repetition_time = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0080), tag_type="float", default=np.nan
            )

            # Echo time
            echo_time = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0081), tag_type="float", default=np.nan
            )

            # Echo train length
            echo_train_length = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0091), tag_type="float", default=np.nan
            )

            # Inversion time
            inversion_time = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0082), tag_type="float", default=np.nan
            )

            # Trigger time
            trigger_time = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x1060), tag_type="float", default=np.nan
            )

            # Contrast/bolus agent
            contrast_agent = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0010), tag_type="str", default=np.nan
            )

            # Magnetic field strength
            magnetic_field_strength = get_pydicom_meta_tag(
                dcm_seq=dcm, tag=(0x0018, 0x0087), tag_type="float", default=np.nan
            )
...
def create_new_uid(dcm: FileDataset):
    ...
    s_keep = [
        s
        for ii, s in enumerate(split_series_uid)
        if s_cum_length[ii] < 64 - min_req_len
    ]
    ...
    random.seed()
    random_string = "".join(
        [str(random.randint(1, 9)) for ii in range(available_length - 1)]
    )
