...
_params_ = [('input_itk_volume', str), ('threshold', float), ('area_closing_radius', float), ('expansion_radius', float), ('output_dir', str), ('save_npy', bool)]

@click.command()
@click.argument('input_itk_volume', nargs=1)
@click.option('-o', '--output_dir', required=False,
              help='path to output directory to save results')
@click.option('-th', '--threshold', required=False,
              help='Threshold above which voxels are labeled')
@click.option('-ar', '--area_closing_radius', required=False,
              help='Radius (of a sphere) of areas to close')
@click.option('-er', '--expansion_radius', required=False,
...
from skimage.morphology import binary_erosion, area_closing, remove_small_holes
from scipy.ndimage import generate_binary_structure
from scipy.ndimage import binary_dilation
from scipy.ndimage import distance_transform_edt 

def generate_threshold_mask(input_itk_volume, threshold, area_closing_radius, expansion_radius, save_npy, output_dir):
    """
    ...
    logger.info(f"area_closing(SA={closing_surface_area}) mask sum = {voxel_grid.sum()}")
