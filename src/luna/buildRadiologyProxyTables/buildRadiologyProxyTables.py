...
def create_proxy_table(config_file):
    ...
    exit_code = 0
    spark = SparkConfig().spark_session(config_file, "luna.radiology.proxy_table.generate")

    # setup for using external py in udf
    sys.path.append("luna/common")
    importlib.import_module("luna.common.EnsureByteContext")
    spark.sparkContext.addPyFile(get_absolute_path(__file__, "../luna-common/EnsureByteContext.py"))
...
def datasets_countDicom(name):
    spark = SparkConfig().spark_session(os.environ['SPARK_CONFIG'], "luna.radiology.proxy_table.generate")
    conn = Neo4jConnection(uri=os.environ["GRAPH_URI"], user="neo4j", pwd="password")
    res = conn.query(f'''MATCH (n:dataset) WHERE n.DATASET_NAME = '{name}' RETURN n''')
