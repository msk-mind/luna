...
class Node(object):
    ...
    def __init__(self, node_type, node_name, properties=None):
        ...
        if (
            "namespace" in self.properties.keys()
            and "subspace" in self.properties.keys()
        ):
            self.properties["qualified_address"] = self.get_qualified_name(
                self.properties["namespace"], self.properties["subspace"], self.name
            )
        elif "namespace" in self.properties.keys():
            self.properties["qualified_address"] = self.get_qualified_name(
                self.properties["namespace"], self.name
            )
        else:
