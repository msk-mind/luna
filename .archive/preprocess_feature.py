...
def process_patient_default(patient: pd.DataFrame) -> pd.DataFrame:
    ...
    :return: None
    """
    scan_absolute_hdfs_path = generate_absolute_path_from_hdfs(
        patient.scan_absolute_hdfs_path.item(), patient.scan_filename.item()
    )
    print("scan path", scan_absolute_hdfs_path)
    annotation_absolute_hdfs_path = generate_absolute_path_from_hdfs(
        patient.annotation_absolute_hdfs_path.item(), patient.annotation_filename.item()
    )
    ...
    target_spacing = (
        patient.preprocessed_target_spacing_x,
        patient.preprocessed_target_spacing_y,
        patient.preprocessed_target_spacing_z,
    )

    if os.path.exists(preprocessed_scan_path) and os.path.exists(
        preprocessed_annotation_path
    ):
        print(
            preprocessed_scan_path
...
def get_dmp_from_scan(conn, query_id):
    result = [
        x.data()
        for x in conn.query(
            f"MATCH (patient:dmp_patient_id)-[:PX_TO_RAD]-(rad)-[:HAS_SCAN]-(scan) WHERE scan.value='{query_id}' RETURN patient, rad, scan"
        )
    ]
    if len(result) >= 1:
...
def cli(
    ...
    with CodeTimer(logger, "generate feature table"):
        ...
        spark = SparkConfig().spark_session(
            config_name=APP_CFG, app_name="preprocess_feature"
        )
...
def generate_feature_table(
    ...
    feature_table = os.path.join(
        destination_directory, "tables/" + str(feature_table_output_name) + "/"
    )
    feature_files = os.path.join(
        destination_directory, "features/" + str(feature_table_output_name) + "/"
    )
    ...
    generate_preprocessed_filename_udf = udf(
        generate_preprocessed_filename, StringType()
    )
    df = annot_df.join(scan_df, ["SeriesInstanceUID"])
    df = df.withColumn(
        "preprocessed_annotation_path",
        lit(
            generate_preprocessed_filename_udf(
                df.annotation_record_uuid, lit("_annotation"), lit(feature_files)
            )
        ),
    )
    df = df.withColumn(
        ...
        lit(
            generate_preprocessed_filename_udf(
                df.scan_record_uuid, lit("_scan"), lit(feature_files)
            )
        ),
    )
    ...
    if query:
        ...
        try:
            ...
            logger.error(
                'Exception while running spark sql query "{}"'.format(sql_query), err
            )
            return
        # If query doesn't return anything, do not proceed.
        if df.count() == 0:
            err_msg = 'query "{}" has no match. Please revisit your query.'.format(
                sql_query
            )
    ...
    if custom_preprocessing_script:
        ...
        custom_preprocessing_script_base = os.path.basename(
            custom_preprocessing_script.replace(".py", "")
        )
        ...
        df = df.groupBy("feature_record_uuid").applyInPandas(
            process_patient_func, schema=df.schema
        )
    else:
        # use default preprocessing function  (process_patient_default)
        df = df.groupBy("feature_record_uuid").applyInPandas(
            process_patient_default, schema=df.schema
        )
    ...
    uid_join_table["dmp_patient_id"] = uid_join_table.apply(
        lambda x: get_dmp_from_scan(conn, x.SeriesInstanceUID), axis=1
    )
