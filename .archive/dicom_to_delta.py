...
def create_delta_table(df, table_name, delta_path, merge, purge):
    ...
    if purge:
        ...
        df.coalesce(128).write.format("delta").option("overwriteSchema", "true").mode(
            "overwrite"
        ).save(delta_path)
    if merge:
        # Adding new columns can be achived with .option("mergeSchema", "true")
        df.coalesce(128).write.format("delta").option("mergeSchema", "true").mode(
            "append"
        ).save(delta_path)

    else:
        df.coalesce(128).write.format("delta").mode("append").save(delta_path)
...
@click.option(
    "-h",
    "--hdfs",
    default="hdfs://sandbox-hdp.hortonworks.com:8020/",
    show_default=True,
    help="HDFS uri e.g. hdfs://sandbox-hdp.hortonworks.com:8020/",
)
@click.option(
    "-a",
    "--dataset_address",
    required=True,
    help="path to dataset directory containing parquet/ and dicom/ directories. This path is the directory for which the new delta table directory /table is created under",
)
...
@click.option(
    ...
    default=False,
    show_default=True,
    help="(optional) Merge schema - add new columns",
)
@click.option(
    "-p",
    "--purge",
    ...
    default=False,
    show_default=True,
    help="(optional) Delete all delta tables - then create new tables",
)
def cli(config_file, hdfs, dataset_address, merge, purge):
    ...
    spark_session = SparkConfig().spark_session(
        config_name=APP_CFG, app_name="dicom-to-delta"
    )
