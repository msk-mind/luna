...
def create_proxy_table(data_config):
    ...
    with CodeTimer(logger, "load scan annotations"):
        ...
        if (
            cfg.has_value(path=const.DATA_CFG + "::METADATA_CSV")
            ...
            annotation_metadata = annotation_metadata.select(
                cfg.get_value(path=const.DATA_CFG + "::METADATA_COLUMNS")
            )
        ...
        df = (
            ...
            .withColumn(
                ...
                generate_uuid_udf(df.path, array([lit("SCAN_ANNOTATION")])),
            )
    ...
    df.coalesce(cfg.get_value(path=const.DATA_CFG + "::NUM_PARTITION")).write.format(
        "delta"
    ...
    logger.info(
        "Processed {} scan annotations out of total {} files".format(
