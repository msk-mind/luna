...
def cli(data_config_file, app_config_file, process_string):
    ...
    with CodeTimer(logger, "generate proxy table"):
        ...
        # load configs
        cfg = ConfigSet(
            name=DATA_CFG, config_file=data_config_file, schema_file=SCHEMA_FILE
        )
...
def transfer_files():
    with CodeTimer(logger, "transfer files"):
        ...
        if exit_code != 0:
            logger.error(
                ("Error Transferring files - Non-zero exit code: " + str(exit_code))
            )
...
def create_proxy_table(config_file):
    ...
    spark = SparkConfig().spark_session(
        config_name=APP_CFG, app_name="luna.radiology.proxy_table.generate"
    )
    ...
    with CodeTimer(logger, "load dicom files"):
        ...
        df = df.withColumn(
            "dicom_record_uuid",
            lit(generate_uuid_udf(df.content, array([lit("DICOM")]))),
        )
    ...
    with CodeTimer(logger, "parse and save dicom"):
        ...
        header.coalesce(cfg.get_value(path=DATA_CFG + "::NUM_PARTITION")).write.format(
            cfg.get_value(path=DATA_CFG + "::FORMAT_TYPE")
        ).mode("overwrite").option("mergeSchema", "true").save(dicom_path)

    processed_count = header.count()
    logger.info(
        "Processed {} dicom headers out of total {} dicom files".format(
            processed_count, cfg.get_value(path=DATA_CFG + "::FILE_COUNT")
        )
    )
    ...
    if processed_count != int(cfg.get_value(path=DATA_CFG + "::FILE_COUNT")):
        exit_code = 1
    df = spark.read.format(cfg.get_value(path=DATA_CFG + "::FORMAT_TYPE")).load(
        dicom_path
    )
    df.printSchema()
    return exit_code
...
def update_graph(config_file):
    ...
    conn = Neo4jConnection(
        uri=cfg.get_value(path=DATA_CFG + "::GRAPH_URI"), user="neo4j", pwd="password"
    )
    ...
    dataset_props = list(
        set(dataset_ext_properties).intersection(set(cfg.get_keys(name=const.DATA_CFG)))
    )

    prop_string = ",".join(
        [
            '''{0}: "{1}"'''.format(prop, cfg.get_value(path=DATA_CFG + "::" + prop))
    ...
    with CodeTimer(logger, "synchronize graph"):
        ...
        for index, row in tuple_to_add.iterrows():
            query = """MATCH (das:dataset {{DATASET_NAME: "{0}"}})
                ...
                MERGE (sc)-[r3:HAS_DATA]->(das)""".format(
                dataset_name,
                row["PatientID"],
                row["AccessionNumber"],
