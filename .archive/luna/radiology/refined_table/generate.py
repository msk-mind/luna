...
@click.option(
    ...
    "--hdfs_uri",
    default="file:///",
    help="hdfs URI uri e.g. hdfs://localhost:8020",
    required=True,
)
@click.option(
    "-c",
    "--custom_preprocessing_script",
    help="Path to python file to execute in the working directory",
    required=True,
)
...
@click.option(
    "-e",
    "--file_ext",
    callback=validate_file_ext,
    help="file format for scan generation",
    required=True,
)
def cli(
    uid, hdfs_uri, custom_preprocessing_script, tag, config_file, project_name, file_ext
):
    """
    ...
    generate_scan_table(
        spark, uid, hdfs_uri, custom_preprocessing_script, tag, project_name, file_ext
    )
...
def generate_scan_table(
    ...
    try:
        df_dcmdata = spark.read.format("delta").load(
            hdfs_uri + os.path.join(project_dir, const.DICOM_TABLE)
        )
    except Exception as ex:
        logger.error(
            "Problem loading dicom table at "
            + hdfs_uri
            + os.path.join(project_dir, const.DICOM_TABLE)
        )
        logger.error(ex)
        exit(1)
    ...
    def python_def_generate_scan(project_dir, file_ext, path):
        ...
        print(
            [bin_python, custom_preprocessing_script, project_dir, input_dir, file_ext]
        )
        ...
        if file_ext == "mhd":
            scan_meta = [
                (scan_record_uuid, filepath + ".mhd", "mhd"),
                (scan_record_uuid, filepath + ".zraw", "zraw"),
            ]
        elif file_ext == "nrrd":
    ...
    df = df_dcmdata.filter(F.col("metadata." + concept_id_type) == uid).limit(1)
    ...
    with CodeTimer(logger, "Generate scans:"):
        ...
        df_scan = df.withColumn(
            "scan_data", udf_generate_scan(F.lit(project_dir), F.lit(file_ext), df.path)
        )
        ...
        if os.path.exists(scan_table_path):
            ...
            intersection = df_existing_scan.join(
                F.broadcast(df_scan), ["scan_record_uuid", "filetype"]
            )

            if intersection.count() == 0:
                df_scan.write.format("delta").mode("append").save(scan_table_path)
        else:
            df_scan.write.format("delta").mode("append").save(scan_table_path)
