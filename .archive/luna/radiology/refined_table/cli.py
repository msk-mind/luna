...
@click.command()
@click.option(
    "-a", "--app_config_file", default="config.yaml", help="application config file"
)
@click.option(
    "-c",
    "--package_config_file",
    default="luna/radiology/refined_table/config.yaml",
...
@click.option(
    ...
    help="A file containing one patient id per line.",
)
@click.option(
    "-i", "--patient_id_type", help="A file containing one patient id per line."
)
...
@click.option(
    ...
    default="luna/radiology/refined_table/dicom_to_scan.py",
...
def cli(
    ...
    conn = Neo4jConnection(
        uri=cfg.get_value(name=const.DATA_CFG, jsonpath="GRAPH_URI"),
    ...
    seriesInstanceUIDs = [
        x.data()["scan"]["SeriesInstanceUID"] for x in conn.query(query)
    ]
