...
def generate_image_table():
    ...
    spark = SparkConfig().spark_session(
        config_name=const.APP_CFG, app_name="dicom-to-png"
    )
    ...
    scan_df = (
        ...
        .load(scan_table_path)
        .select(
            F.col("metadata").alias("scan_metadata"),
            F.col("path").alias("scan_path"),
    ...
    with CodeTimer(logger, "Generate pngs and seg_png table"):
        ...
        n_slices = validate_integer_param(
            cfg.get_value(path=const.DATA_CFG + "::N_SLICES")
        )
        crop_width = validate_integer_param(
            cfg.get_value(path=const.DATA_CFG + "::CROP_WIDTH")
        )
        crop_height = validate_integer_param(
            cfg.get_value(path=const.DATA_CFG + "::CROP_HEIGHT")
        )
        ...
        spark.sparkContext.addPyFile(
            get_absolute_path(__file__, "../../common/preprocess.py")
        )
        from preprocess import create_images

        create_images_udf = F.udf(
            create_images,
            ArrayType(
                StructType(
                    [
        ...
        columns_to_select = [
            F.col("slices_images.n_tumor_slices").alias("n_tumor_slices"),
            ...
            "path",
            "subset_path",
            "scan_annotation_record_uuid",
        ]
        columns_to_select.extend(metadata_columns)

        seg_df = seg_df.withColumn("slices_images", F.explode("slices_images")).select(
            columns_to_select
        )
        ...
        # generate uuid
        spark.sparkContext.addPyFile(
            get_absolute_path(__file__, "../../../../../src/luna/common/utils.py")
        )
        ...
        seg_df = seg_df.withColumn(
            "png_record_uuid",
            F.lit(generate_uuid_udf(seg_df.overlay, F.array([F.lit("PNG")]))),
        )

        seg_df.coalesce(
            cfg.get_value(path=const.DATA_CFG + "::NUM_PARTITION")
        ).write.format("delta").mode("overwrite").save(seg_png_table_path)
