...
def binary_to_png(cfg):
    ...
    multiple_annotations = (
        df.groupby("metadata.AccessionNumber")
    ...
    for index, row in df.toPandas().iterrows():
        ...
        image = Image.frombytes(
            mode, (IMAGE_WIDTH, IMAGE_HEIGHT), bytes(row[COLUMN_NAME])
        )

        image_dir = os.path.join(
            DESTINATION_PATH, COLUMN_NAME, row["metadata"]["AccessionNumber"]
        )
        ...
        if row["metadata"]["AccessionNumber"] in multiple_cases and row.label:
            image_dir = os.path.join(
                DESTINATION_PATH,
                COLUMN_NAME,
                row["metadata"]["AccessionNumber"] + "_" + row.label,
            )
