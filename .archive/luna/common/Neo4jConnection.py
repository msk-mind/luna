...
class Neo4jConnection:
    ...
    def create_id_lookup_table_where(
        self, sqlc, source, sink, r="ID_LINK|HAS_RECORD", WHERE_CLAUSE=""
    ):
        """
        ...
        if re.compile("|".join(banned_words), re.IGNORECASE).search(
            WHERE_CLAUSE
        ...
        cSchema = StructType(
            [
                ...
                StructField("pathspec", StringType(), True),
            ]
        )
        return sqlc.createDataFrame(
            (
                [
                    (
                        x.data()["source"]["value"],
                        x.data()["sink"]["value"],
                        pretty_path(x.data()["path"]),
                    )
                    for x in result
                ]
            ),
    ...
    def commute_source_id_to_spark(self, sc, sqlc, SOURCE_TYPE, SINK_TYPE, QUERY_ID):
        ...
        return sqlc.createDataFrame(
            sc.parallelize([(x.data()["sink"]["value"],) for x in result]),
            schema=cSchema,
        )
    ...
    def commute_source_id_to_spark_query(self, sc, sqlc, WHERE_CLAUSE, SINK_TYPE):
        ...
        for x in result:
            print(pretty_path(x.data()["path"]))
        cSchema = StructType([StructField(SINK_TYPE, StringType(), True)])
        return sqlc.createDataFrame(
            ([(x.data()["sink"]["value"],) for x in result]), schema=cSchema
        )
    ...
    def create_id_lookup_table(self, sc, sqlc, SOURCE_TYPE, SINK_TYPE, QUERY_ID):
        ...
        return sqlc.createDataFrame(
            sc.parallelize(
                [
                    (x.data()["source"]["value"], x.data()["sink"]["value"])
                    for x in result
                ]
            ),
            schema=cSchema,
    ...
    def commute_sink_id_to_spark(self, sc, sqlc, SINK_TYPE, QUERY_ID):
        ...
        cSchema = StructType([StructField(SINK_TYPE, StringType(), True)])
        return sqlc.createDataFrame(
            sc.parallelize([(x,) for x in [QUERY_ID]]), schema=cSchema
        )
