...
def create_geojson_table():
    ...
    df = (
        df.withColumn("label_config", lit(str(label_config)))
    ...
    df = df.groupby(["bmp_record_uuid", "labelset"]).applyInPandas(
        build_geojson_from_annotation, schema=df.schema
    )
    ...
    df = df.withColumn(
        "geojson_record_uuid",
        geojson_record_uuid_udf("geojson", array(lit("SVGEOJSON"), "labelset")),
    )
    ...
    geojson_df = (
        geojson_df.withColumn("latest", lit(True))
        .withColumn("date_added", current_timestamp())
        .withColumn("date_updated", current_timestamp())
    )
    # update geojson delta table
    geojson_df.write.format("delta").mode("overwrite").option(
        "overwriteSchema", "true"
    ).save(geojson_table_path)
...
def create_concat_geojson_table():
    ...
    concatgeojson_df = (
        concatgeojson_df.select(
            "sv_project_id", "slideviewer_path", "slide_id", "labelset", "geojson"
        )
    ...
    concatenate_regional_geojsons_udf = udf(
        concatenate_regional_geojsons, geojson_struct
    )
    concat_geojson_record_uuid_udf = udf(generate_uuid_dict, StringType())

    # cache to not have udf called multiple times
    concatgeojson_df = concatgeojson_df.withColumn(
        "concat_geojson", concatenate_regional_geojsons_udf("geojson_list")
    ).cache()

    concatgeojson_df = concatgeojson_df.drop("geojson_list").withColumn(
        "concat_geojson_record_uuid",
        concat_geojson_record_uuid_udf(
            to_json("concat_geojson"), array(lit("SVCONCATGEOJSON"), "labelset")
        ),
    )

    concatgeojson_df = (
        concatgeojson_df.withColumn("latest", lit(True))
        .withColumn("date_added", current_timestamp())
        .withColumn("date_updated", current_timestamp())
    )

    # update concatenation geojson delta table
    concatgeojson_df.write.format("delta").mode("overwrite").option(
        "overwriteSchema", "true"
    ).save(concat_geojson_table_path)
