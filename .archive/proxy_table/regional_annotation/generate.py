...
DATA_SCHEMA_FILE = os.path.join(
    pathlib.Path(__file__).resolve().parent, "data_config_schema.yml"
)
...
def process_regional_annotation_slide_row_pandas(row: pd.DataFrame) -> pd.DataFrame:
    ...
    bmp_dirname = os.path.join(
        row.SLIDE_BMP_DIR.item(), full_filename_without_ext.replace(";", "_")
    )
    ...
    url = (
        row.SLIDEVIEWER_API_URL.item()
        + "slides/"
        + str(user)
        + "@mskcc.org/projects;"
        + str(row.sv_project_id.item())
        + ";"
        + full_filename
        + "/getLabelFileBMP"
    )
...
def create_proxy_table():
    ...
    df = (
        df.withColumn("bmp_filepath", lit(""))
        .withColumn(
            "users",
        ...
        .withColumn(
            "SLIDEVIEWER_API_URL",
            lit(cfg.get_value(const.DATA_CFG + "::SLIDEVIEWER_API_URL")),
        )
    )
    ...
    df = df.groupby(["slideviewer_path", "user"]).applyInPandas(
        process_regional_annotation_slide_row_pandas, schema=df.schema
    )
    df.show()
    ...
    # convert to numpy
    df["npy_filepath"] = df.apply(
        lambda x: convert_bmp_to_npy(x.bmp_filepath, SLIDE_NPY_DIR), axis=1
    )
    ...
    if not os.path.exists(BITMASK_TABLE_PATH):
        ...
        bitmask_table.alias("main_bitmask_table").merge(
            spark_bitmask_df.alias("bmp_annotation_updates"),
            "main_bitmask_table.bmp_record_uuid = bmp_annotation_updates.bmp_record_uuid",
        ).whenMatchedUpdate(
            set={"date_updated": "bmp_annotation_updates.date_updated"}
