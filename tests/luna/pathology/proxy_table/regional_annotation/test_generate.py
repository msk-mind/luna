...
from luna.pathology.proxy_table.regional_annotation.generate import cli, convert_bmp_to_npy, \
...
def test_convert_bmp_to_npy():
    actual_path = convert_bmp_to_npy('tests/luna/pathology/proxy_table/'
...
def test_process_regional_annotation_slide_row_pandas(monkeypatch, requests_mock):
    ...
    requests_mock.get("https://fakeslides-res.mskcc.org/slides/someuser@mskcc.org/projects;155;CMU-1.svs/getLabelFileBMP",
                      content=Path('tests/luna/pathology/proxy_table/regional_annotation/test_data/input/CMU-1.zip').read_bytes())
    ...
    data = {'slideviewer_path': ['CMU-1.svs'],
            ...
            'bmp_record_uuid': [''],
            'latest': [True],
            'SLIDE_BMP_DIR': ['tests/luna/pathology/proxy_table/regional_annotation/test_data/output/regional_bmps'],
            'TMP_ZIP_DIR': ['tests/luna/pathology/proxy_table/regional_annotation/test_data/output/gynocology_tmp_zips'],
    ...
    assert df['bmp_filepath'].item() == 'tests/luna/pathology/proxy_table/regional_annotation' \
...
def test_create_proxy_table(monkeypatch):
    ...
    def mock_process(row:  pandas.DataFrame)-> pandas.DataFrame:
        data = {'slideviewer_path': ['CMU-1.svs'],
                'slide_id': ['CMU-1'],
                'sv_project_id': [155],
                'bmp_filepath': ['tests/luna/pathology/proxy_table/regional_annotation/test_data/input/labels.bmp'],
                'user': ['someuser'],
                ...
                'latest': [True],
                'SLIDE_BMP_DIR': [
                    'tests/luna/pathology/proxy_table/regional_annotation/test_data/output/regional_bmps'],
                'TMP_ZIP_DIR': [
                    'tests/luna/pathology/proxy_table/regional_annotation/test_data/output/gynocology_tmp_zips'],
                'SLIDEVIEWER_API_URL': ['https://fakeslides-res.mskcc.org/']}
