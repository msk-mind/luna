...
from luna.pathology.refined_table.regional_annotation.generate import cli
import luna.common.constants as const
# from luna.pathology.common.build_geojson import build_geojson_from_annotation

from pyspark.sql.functions import  to_json, lit, collect_list, udf

project_path = "tests/luna/pathology/testdata/data/test-project"
...
def test_cli_geojson(spark):
    ...
    result = runner.invoke(cli, 
        ['-d', 'tests/luna/pathology/refined_table/regional_annotation/geojson_data.yaml',
...
def test_cli_concat(spark):
    ...
    runner = CliRunner()
    result = runner.invoke(cli,
                           ['-d', 'tests/luna/pathology/refined_table/regional_annotation/geojson_concat_data.yaml',
                            '-a', 'tests/test_config.yml',
                            '-p', 'concat'])
...
def test_hole_geojson(monkeypatch, spark):
    ...
    # import setup
    spark.sparkContext.addPyFile("./luna/pathology/common/build_geojson.py")
    from build_geojson import build_geojson_from_annotation
    from build_geojson import concatenate_regional_geojsons
    from luna.pathology.refined_table.regional_annotation.generate import geojson_struct


    data = {'slideviewer_path': ['TEST-1.svs'],
            'slide_id': ['TEST-1'],
            'sv_project_id': [155],
            'npy_filepath': ['tests/luna/pathology/refined_table/regional_annotation/test_data/small_holed_annotation.npy'],
            'user': ['someuser'],
