...
scan_table_path = os.path.join("tests/luna/testdata/data", project_name, "tables/scan")
APP_CFG='APP_CFG'
...
def spark(monkeypatch):
    print('------setup------')
    # setup env
    monkeypatch.setenv("MIND_ROOT_DIR", os.path.join(current_dir, "tests/luna/testdata/data"))
...
def test_cli(mocker, spark):
    ...
    runner = CliRunner()
    generate_mhd_script_path = os.path.join(current_dir, "luna/radiology/refined_table/dicom_to_scan.py")
...
#    mocker.patch('luna.radiology.refined_table.generate.generate_scan_table.python_def_generate_scan.os.path.split', side_effect=['/home/circleci/project/tests/luna/testdata/data/test-project/dicoms/RIA_16_158A_000013/20150902_CT/2_Standard_5mm/'])
