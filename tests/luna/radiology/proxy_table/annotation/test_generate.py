...
project_path = "tests/luna/radiology/testdata/test-project"
...
def test_cli_mha(spark):
    ...
    result = runner.invoke(cli, 
        ['-d', 'tests/luna/radiology/proxy_table/annotation/mha_data_config.yaml',
...
def test_cli_both(spark):
    ...
    runner = CliRunner()
    result = runner.invoke(cli, 
        ['-d', 'tests/luna/radiology/proxy_table/annotation/data_config.yaml',
        '-a', 'tests/test_config.yml',
        '-p', 'delta'])
    assert result.exit_code == 0
