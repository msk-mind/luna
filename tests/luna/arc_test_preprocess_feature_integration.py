...
def spark(monkeypatch):
    ...
    spark = SparkConfig().spark_session(
        config_name=APP_CFG, app_name="test-preprocessing-feature"
    )
...
def test_local_feature_table_generation(mocker, spark):
    ...
    generate_feature_table(
        ...
        TARGET_SPACING,
        spark,
        "SeriesInstanceUID = '1.2.840.113619.2.55.3.2743925538.934.1319713655.582'",
        "feature-table-test-name",
        "tests/external_process_patient_script.py",
    )
...
def test_local_feature_table_generation_malformed_query(spark):
    ...
    generate_feature_table(
        ...
        TARGET_SPACING,
        spark,
        "SeriesInstanceUID = '123' || scan_record_uuid = '123'",
        "feature-table",
        "tests/test_external_process_patient_script.py",
    )
...
def test_feature_table_cli():
    result = runner.invoke(
        cli,
        "--base_directory {} --target_spacing 1 1 3 --query \"SeriesInstanceUID = '1.2.840.113619.2.55.3.2743925538.934.1319713655.579'\" --feature_table_output_name test-cli-2 --custom_preprocessing_script tests/test_external_process_patient_script.py --config_file tests/test_config.yml".format(
            BASE_DIR
        ),
    )
...
def test_feature_table_cli_with_destination_directory():
    ...
    result = runner.invoke(
        cli,
        "--base_directory {0} --destination_directory {1} --target_spacing 1 1 3 --query \"SeriesInstanceUID = '1.2.840.113619.2.55.3.2743925538.934.1319713655.579'\" --feature_table_output_name test-cli-2 --custom_preprocessing_script tests/test_external_process_patient_script.py --config_file tests/test_config.yml".format(
...
def test_feature_table_cli_with_config():
    result = runner.invoke(
        cli,
        "--base_directory {} --target_spacing 1 1 3 --query \"SeriesInstanceUID = '1.2.840.113619.2.55.3.2743925538.934.1319713655.579'\" --feature_table_output_name test-cli-2 --custom_preprocessing_script tests/test_external_process_patient_script.py -f tests/test_config.yml".format(
            BASE_DIR
        ),
    )
    assert result.exit_code == 0
