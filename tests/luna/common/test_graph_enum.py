...
from luna.common.Node import ALL_DATA_TYPES, CONTAINER_TYPES, Node
...
def test_patient_create():
    create_string = Node(
        "patient",
        "my_patient",
        properties={"namespace": "my_cohort", "Description": "a patient"},
    ).get_create_str()
...
def test_cohort_create():
    create_string = Node(
        "cohort", "my_cohort", properties={"Description": "a cohort"}
    ).get_create_str()
...
def test_metadata_create():
    ...
    create_string = Node(
        "VolumetricImage", "SCAN-001", properties=properties
    ).get_create_str()
...
def test_metadata_match():
    ...
    match_string = Node(
        "VolumetricImage", "SCAN-002", properties=properties
    ).get_match_str()
